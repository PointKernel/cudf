...
- Use `std::span`/`cuda::std::span`, or `host_span` when device-accessibility metadata is needed;
  no owning vectors passed by copy/reference unless explicitly moved
